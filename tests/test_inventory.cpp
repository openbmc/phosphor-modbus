...
class InventoryTest : public BaseTest
{
  public:
    ...
    {
        auto [mockPort, inventoryDevice] = createDevice(testProfile);
    ...
    auto testDormantSkippedAndExpires() -> sdbusplus::async::task<void>
    {
        ...
        auto [mockPort,
              inventoryDevice] = createDevice(failProfile, testDormantPeriod);
...
TEST_F(InventoryTest, TestNonRespondingAddressNoInventoryObject)
{
    auto testProbe = [&]() -> sdbusplus::async::task<void> {
        auto [mockPort, inventoryDevice] = createDevice(failProfile);
...
{
    auto testProbe = [&]() -> sdbusplus::async::task<void> {
        auto [mockPort, inventoryDevice] = createDevice(flakyProfile);
...
{
    auto testProbe = [&]() -> sdbusplus::async::task<void> {
        auto [mockPort, inventoryDevice] = createDevice(mismatchProfile);
...
{
    auto testProbe = [&]() -> sdbusplus::async::task<void> {
        auto [mockPort, inventoryDevice] = createDevice(nullPaddedProfile);
...
TEST_F(InventoryTest, TestStopDeviceExitsAndCleansUp)
{
    auto testStop = [&]() -> sdbusplus::async::task<void> {
        ...
        co_await sdbusplus::async::sleep_for(ctx, 5s);

        EXPECT_TRUE(inventoryDevice->isFinished())
    ...
    ctx.spawn(sdbusplus::async::sleep_for(ctx, 8s) |
              sdbusplus::async::execution::then([&]() { ctx.request_stop(); }));
