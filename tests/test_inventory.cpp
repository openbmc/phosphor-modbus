  ...
  public:
                     ...
                     std::unique_ptr<InventoryIntf::Device>>
    {
        ...
        auto device = std::make_unique<InventoryIntf::Device>(
            ctx, baseConfig, *mockPort, allowedDevices, nullptr, dormantPeriod);
        ...
        -> sdbusplus::async::task<void>
    {
        ...
        // Stop probing and wait for the coroutine to stop before the
        // device is destroyed
        inventoryDevice->requestStop();
...
// Verify that calling requestStop() causes the probing coroutine to exit,
// sets isStopped() to true, and cleans up the inventory D-Bus object.
TEST_F(InventoryTest, TestStopDeviceExitsAndCleansUp)
{
    auto testStop = [&]() -> sdbusplus::async::task<void> {
        ...
        EXPECT_FALSE(inventoryDevice->isStopped())
            << "Device should not be stopped yet";
