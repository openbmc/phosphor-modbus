...
auto toJson(const RegisterDump& reg) -> nlohmann::json
{
    nlohmann::json out;
    out["Name"] = reg.name;
    ...
    if (!reg.bits.empty())
    {
        ...
        {
            bits.emplace_back(nlohmann::json{{"Position", bit.position},
                                             {"Name", bit.name},
                                             {"Type", bit.type},
...
auto toJson(const std::vector<RegisterDump>& registers) -> nlohmann::json
{
...
auto toJson(const DeviceDump& device) -> nlohmann::json
{
    nlohmann::json out;
