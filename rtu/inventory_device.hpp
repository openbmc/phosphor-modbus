...
struct Config
{
    using address_range_t = std::tuple<uint8_t, uint8_t>; // <start, end>
    using address_range_arr_t = std::vector<address_range_t>;
    using port_address_map_t =
        std::map<std::string,
                 address_range_arr_t>; // <port, device address range list>
...
class Device
{
  ...
  private:
    ...
    const config::Config config;
    serial_port_map_t& serialPorts;
    std::map<uint8_t, std::unique_ptr<InventorySourceIntf>> inventorySources;
};
