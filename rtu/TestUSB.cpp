...
auto getUSBDevicePath(const std::string& deviceAddress, uint8_t interface, uint8_t port) -> std::optional<std::string>
{
  namespace fs = std::filesystem;
  std::regex pattern(std::format("platform-{}\\.usb-usb.*{}-port{}", deviceAddress, interface, port));
  ...
  for (const auto& entry : fs::recursive_directory_iterator(searchDir)) 
  {
    if (entry.is_symlink())
    {
      ...
      {
        return ("/dev/" + fs::read_symlink(filePath).filename().string());
      }
    }
  }
