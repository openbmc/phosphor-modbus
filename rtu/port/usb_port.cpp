    ...
    -> std::string
{
    ...
    if (config.hubport)
    {
        ...
        spattern = std::format(
            "platform-{}\\.usb-usb-[0-9]:[0-9]\\.{}:[0-9]\\.{}-port{}",
            config.address, config.hubport, config.interface, config.port);
    }
    else
    {
        // platform-1e6a1000.usb-usb-0:1:1.0-port0
        spattern =
            std::format("platform-{}\\.usb-usb-[0-9]:[0-9]:[0-9]\\.{}-port{}",
                        config.address, config.interface, config.port);
    }
