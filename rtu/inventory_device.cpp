    ...
    Config& config, const InventoryBaseConfigMap& configMap) -> bool
{
    ...
    info("ModbusRegisters {NAME} {ADDRESS} {SIZE}", "NAME",
         registerConfig.name, "ADDRESS", registerConfig.offset, "SIZE", registerConfig.size);
...
static auto printConfig(const Config& config) -> void
{
    ...
    {
        info("Inventory device config: {NAME} {ADDRESS} {SIZE}",
             "NAME", registerConfig.name, "ADDRESS", registerConfig.offset, "SIZE",
             registerConfig.size);
    }
}
    ...
    -> sdbusplus::async::task<std::optional<Config>>
{
                                                        ...
                                                        "GetManagedObjects"))
    {
        ...
        {
            info("Skipping device {PATH}", "PATH", path.str);
            continue;
        }
