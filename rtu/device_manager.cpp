    ...
    const std::string& interfaceName) -> sdbusplus::async::task<>
{
    ...
    {
        co_await processPortAdded(objectPath, interfaceName);
    }
...
auto DeviceManager::processPortAdded(
    ...
    const std::string& interfaceName) -> sdbusplus::async::task<>
{
    auto res = co_await PortIntf::PortFactory::getConfig(ctx, objectPath,
                                                         interfaceName);
    if (!res)
    {
    ...
    const std::string& interfaceName) -> sdbusplus::async::task<>
{
    ...
    try
    {
        ...
        /*
                ...
                {
                    auto inventoryDevice =
                        std::make_unique<InventoryIntf::Device>(ctx, config,
           ports); ctx.spawn(inventoryDevice->probePorts());
                    inventoryDevices[config.name] = std::move(inventoryDevice);
                }
                catch (const std::exception& e)
                {
                    error("Failed to create Inventory Device for {PATH} with
           {ERROR}", "PATH", objectPath, "ERROR", e); co_return;
                }
        */
