...
static inline auto getDataParity(Config& config,
    ...
    -> bool
{
    auto parityIter = configMap.find("DataParity");
    if (parityIter == configMap.end())
    {
        error("Missing DataParity for {NAME}", "NAME", config.name);
        return false;
    }
    auto receivedParity = std::get<std::string>(parityIter->second);
    ...
    -> bool
{
    ...
    auto baudRateIter = configMap.find("BaudRate");
    if (baudRateIter == configMap.end())
    {
        error("Missing BaudRate for {NAME}", "NAME", config.name);
        return false;
    }
    config.baudRate = std::get<uint64_t>(baudRateIter->second);
...
auto BaseDevice::createSensors() -> sdbusplus::async::task<void>
{
    for (const auto& sensorRegister : config.sensorRegisters)
    {
        ...
        auto sensorPath = getObjectPath(
            sensorRegister.pathSuffix + "/" + sensorRegister.name);
